...
public class CommandReload implements CommandSub {
    ...
    public CommandReload(SunBlastTierTagger plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length >= 1) {
            ...
            if (!Utils.has(player, "SunBlastTierTagger.reload")) {
                return true;
            } else {
                if (args.length == 0) {
                    Utils.reloadConfig();
                    Utils.sendMessage(player, Utils.getMessage("messages.commands.reload.successfully"));
                    return true;
                } else {
                    Utils.sendMessage(player, Utils.getMessage("messages.commands.reload.error"));
                }
                return true;
            }
        }
    ...
    public String permission() {
        return "SunBlastTierTagger.reload";
    }

    @Override
    public String description() {
        return Utils.getMessage("messages.commands.reload.usage");
    }
}